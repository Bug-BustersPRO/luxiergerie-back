...
public class PurchaseService {
    ...
    public PurchaseService(PurchaseRepository purchaseRepository, RoomRepository roomRepository, AccommodationRepository accommodationRepository) {
    ...
    public PurchaseDTO createPurchase(PurchaseDTO purchaseDTO) {
        ...
        // Map le DTO en entité Purchase
        Purchase purchase = PurchaseMapper.MappedPurchaseFrom(purchaseDTO);

        // Réattacher les entités Accommodation au contexte de persistance
        List<Accommodation> attachedAccommodations = new ArrayList<>();
        for (Accommodation accommodation : purchase.getAccommodations()) {
            Accommodation managedAccommodation = accommodationRepository.findById(accommodation.getId())
                    .orElseThrow(() -> new IllegalArgumentException("Accommodation not found"));
            attachedAccommodations.add(managedAccommodation);
        }
        purchase.setAccommodations(attachedAccommodations);

        // Persister l'entité Purchase
        Purchase newPurchase = purchaseRepository.save(purchase);

        // Retourner le DTO correspondant
        return PurchaseMapper.MappedPurchaseFrom(newPurchase);
    }
